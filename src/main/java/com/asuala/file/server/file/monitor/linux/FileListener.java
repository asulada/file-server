...
public class FileListener {
    ...
    // 提取获取父目录ID的公共方法
    private Long getParentDirId(FileVo poll) {
        FileMemory fileMemory = InotifyLibraryUtil.fdMap.get(poll.getFd()).getPathIdMap().get(poll.getParentPath());
        if (null == fileMemory) {
            FileInfo parentInfo = fileInfoService.findFileInfo(new File(poll.getParentPath()));
            return parentInfo != null ? parentInfo.getDId() : -1;
        } else {
            return fileMemory.getDId();
        }
    }
    @PostConstruct
    public void consumer() {
        ...
        fixedThreadPool.execute(() -> {
            while (CacheUtils.watchFlag) {
                FileVo poll = new FileVo();
                try {
                    while (CacheUtils.watchFlag) {
                        poll = (FileVo) CacheUtils.queue.poll();
                        if (null == poll) {
                            Thread.sleep(1000L);
                        } else {
